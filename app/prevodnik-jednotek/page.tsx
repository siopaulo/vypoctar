...
export default function UnitConverterPage() {
  ...
  const [fromUnit, setFromUnit] = useState(units[category][2]) // Metr
  const [toUnit, setToUnit] = useState(units[category][2]) // Metr
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-grow">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 mb-8">
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Převodník jednotek</h1>
          </div>
          ...
          {result !== null && (
            <div className="mt-8 p-6 bg-primary-50 dark:bg-primary-900/20 rounded-lg">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Výsledek:</p>
              <p className="text-3xl font-bold text-primary-700 dark:text-primary-400">
                {result.toLocaleString('cs-CZ', { maximumFractionDigits: 6 })} {toUnit.symbol}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                {value} {fromUnit.symbol} = {result.toLocaleString('cs-CZ', { maximumFractionDigits: 6 })} {toUnit.symbol}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">O převodníku jednotek</h2>
          <p className="text-gray-600 dark:text-gray-300 mb-4">
            Tento převodník umožňuje převádět mezi různými jednotkami délky, hmotnosti, objemu, plochy a teploty.
            Všechny převody jsou přesné a založené na mezinárodních standardech.
          </p>
          <ul className="list-disc list-inside text-gray-600 dark:text-gray-300 space-y-2">
            <li>Délka: milimetry, centimetry, metry, kilometry, palce, stopy, yardy, míle</li>
            <li>Hmotnost: gramy, kilogramy, tuny, unce, libry</li>
            <li>Objem: mililitry, litry, kubické metry, galony, pinty</li>
            <li>Plocha: čtvereční metry, kilometry, hektary, akry</li>
            <li>Teplota: Celsius, Fahrenheit, Kelvin</li>
          </ul>
        </div>
      </main>
    </div>
