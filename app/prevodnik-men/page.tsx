...
export default function CurrencyConverterPage() {
  const [fromCurrency, setFromCurrency] = useState(currencies[0]) // CZK
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex flex-col">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-grow">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 mb-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Převodník měn</h1>
            </div>
            <button
              onClick={fetchRate}
              disabled={loading}
          ...
          {rate !== null && (
            <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <p className="text-sm text-gray-600 dark:text-gray-300">
                Aktuální kurz: 1 {fromCurrency.code} = {rate.toFixed(4)} {toCurrency.code}
              </p>
            </div>
          )}
          ...
          {result !== null && (
            <div className="mt-8 p-6 bg-green-50 dark:bg-green-900/20 rounded-lg">
              ...
              <p className="text-3xl font-bold text-green-700 dark:text-green-400">
                {result.toLocaleString('cs-CZ', { maximumFractionDigits: 2 })} {toCurrency.symbol}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                {amount} {fromCurrency.symbol} = {result.toLocaleString('cs-CZ', { maximumFractionDigits: 2 })} {toCurrency.symbol}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">O převodníku měn</h2>
          <p className="text-gray-600 dark:text-gray-300 mb-4">
            Tento převodník používá aktuální kurzy měn z veřejného API. Kurzy se aktualizují automaticky
            a jsou založené na reálných tržních hodnotách.
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Poznámka: Kurzy jsou orientační. Pro přesné kurzy pro bankovní převody použijte oficiální zdroje.
          </p>
        </div>
      </main>
    </div>
